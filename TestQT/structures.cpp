...
void ThemMonHoc(treeMH &root, MonHoc mh) {
    if (root == nullptr) {
        nodeMH* p = new nodeMH{mh, nullptr, nullptr};
        root = p;
        return;
    }
    if (strcmp(mh.MAMH, root->mh.MAMH) < 0)
...
nodeMH* TimMonHoc(treeMH root, const char *ma) {
    if (root == nullptr) return nullptr;
    int cmp = strcmp(ma, root->mh.MAMH);
    if (cmp == 0) return root;
    else if (cmp < 0) return TimMonHoc(root->left, ma);
    else return TimMonHoc(root->right, ma);
}

bool XoaMonHoc(treeMH &root, const char *ma) {
    if (root == nullptr) return false;
    int cmp = strcmp(ma, root->mh.MAMH);
    if (cmp < 0) return XoaMonHoc(root->left, ma);
    else if (cmp > 0) return XoaMonHoc(root->right, ma);
    else {
        nodeMH* temp;
        if (root->left == nullptr) {
            ...
            root->mh = p->mh;
            XoaMonHoc(root->right, p->mh.MAMH);
        }
        return true;
    }
}

void HieuChinhMonHoc(nodeMH* p, MonHoc newMH) {
    if (p) {
        strcpy(p->mh.TENMH, newMH.TENMH);
        p->mh.STCLT = newMH.STCLT;
        p->mh.STCTH = newMH.STCTH;
    }
}

void LNR_InDanhSach(treeMH root, QTextStream &out) {
    if (root) {
        LNR_InDanhSach(root->left, out);
        out << root->mh.MAMH << " - " << root->mh.TENMH
            << " (" << root->mh.STCLT << " LT, " << root->mh.STCTH << " TH)\n";
        LNR_InDanhSach(root->right, out);
    }
}

void GhiDanhSachMonHoc(QFile &file, treeMH root) {
    if (!root) return;
    QTextStream out(&file);
    out << root->mh.MAMH << "|" << root->mh.TENMH << "|"
        << root->mh.STCLT << "|" << root->mh.STCTH << "\n";
    GhiDanhSachMonHoc(file, root->left);
    GhiDanhSachMonHoc(file, root->right);
}

void DocDanhSachMonHoc(QFile &file, treeMH &root) {
    root = nullptr;
    QTextStream in(&file);
    while (!in.atEnd()) {
        ...
        strcpy(mh.MAMH, parts[0].toStdString().c_str());
        strcpy(mh.TENMH, parts[1].toStdString().c_str());
