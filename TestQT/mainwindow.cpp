...
void MainWindow::ghiDuLieuRaFile() {
    QFile file("DSMH.txt");
    if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        GhiDanhSachMonHoc(file, dsMH);
        file.close();
    }
}
...
void MainWindow::on_btnThemMH_clicked() {
    MonHoc mh;
    strcpy(mh.MAMH, ui->txtMaMH->text().toStdString().c_str());
    strcpy(mh.TENMH, ui->txtTenMH->text().toStdString().c_str());
    mh.STCLT = ui->txtSTCLT->text().toInt();
    mh.STCTH = ui->txtSTCTH->text().toInt();
    if (TimMonHoc(dsMH, mh.MAMH)) {
        QMessageBox::warning(this, "Lỗi", "Mã môn học đã tồn tại!");
        return;
    }
    ThemMonHoc(dsMH, mh);
...
void MainWindow::on_btnXoaMH_clicked() {
    QString ma = ui->txtMaMH->text();
    if (XoaMonHoc(dsMH, ma.toStdString().c_str()))
        QMessageBox::information(this, "OK", "Đã xóa!");
    else
...
void MainWindow::on_btnSuaMH_clicked() {
    QString ma = ui->txtMaMH->text();
    nodeMH* p = TimMonHoc(dsMH, ma.toStdString().c_str());
    if (!p) {
        QMessageBox::warning(this, "Lỗi", "Không tìm thấy môn học!");
        return;
    }
    MonHoc newMH;
    strcpy(newMH.MAMH, p->mh.MAMH); // giữ nguyên mã
    strcpy(newMH.TENMH, ui->txtTenMH->text().toStdString().c_str());
    newMH.STCLT = ui->txtSTCLT->text().toInt();
    newMH.STCTH = ui->txtSTCTH->text().toInt();
    HieuChinhMonHoc(p, newMH);
    QMessageBox::information(this, "OK", "Đã sửa!");
}

void MainWindow::on_btnInDSMH_clicked() {
    ui->txtKetQuaMonHoc->clear();
    QString text;
